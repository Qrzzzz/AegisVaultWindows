...
def test_main_window_has_only_three_primary_workspaces(tmp_path: Path) -> None:
    ...
    assert window.shell.stack.count() == 3
    assert len(window.shell.navigation.buttons) == 3
    for index in range(3):
        QTest.mouseClick(window.shell.navigation.buttons[index], Qt.MouseButton.LeftButton)
        app.processEvents()
        assert window.shell.stack.currentIndex() == index
        assert window.shell.navigation.buttons[index].isChecked()
...
def test_settings_apply_in_place_without_losing_workspace_state(tmp_path: Path) -> None:
    ...
    assert window.shell.navigation.buttons[0].text() == "Text"
