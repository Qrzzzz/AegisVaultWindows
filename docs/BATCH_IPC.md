...
In 2.8 the desktop progress bar describes the current single-file request. Separate counts show attempts processed in this run and files still waiting, including live additions/removals. The backend's multi-item aggregate progress contract above is unchanged. The failed-only view does not alter the processing queue; bulk requeue touches only failures and clearing completed entries removes UI records without deleting output files.
