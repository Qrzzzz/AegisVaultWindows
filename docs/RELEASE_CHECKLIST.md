...
- Confirm `pyproject.toml`, `src/aegisvault/version.py`, README, SECURITY, CHANGELOG, QA checklist and release notes all reference `1.1.0` consistently.
...
- Confirm the PE is AMD64/PE32+ GUI and contains icon, group-icon, manifest and exact `1.1.0.0` version resources.
- Confirm `dist\AegisVault-v1.1.0-win64.zip` contains exactly `AegisVault.exe` with identical bytes.
- Confirm `dist\AegisVault-v1.1.0.cdx.json` is CycloneDX JSON 1.6 and binds the source commit plus executable/ZIP digests.
...
- Create an annotated strict SemVer tag such as `git tag -a v1.1.0 <merge-sha> -m "AegisVault v1.1.0"`.
