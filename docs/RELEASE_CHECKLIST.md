...
- Confirm `pyproject.toml`, `src/aegisvault/version.py`, README, SECURITY, CHANGELOG, QA checklist and release notes all reference `2.5` consistently.
...
- Confirm the PE is AMD64/PE32+ GUI and contains icon, group-icon, manifest and exact `2.5.0.0` version resources.
- Confirm `dist\AegisVault-v2.5-win64.zip` contains the complete self-contained WinUI and backend folder with identical bytes.
- Confirm `dist\AegisVault-v2.5.cdx.json` is CycloneDX JSON 1.6 and binds the source commit plus executable/ZIP digests.
...
- Create an annotated two-component tag such as `git tag -a v2.5 <merge-sha> -m "AegisVault v2.5"`.
...
- `AegisVault-v2.5-win64.zip`
- `AegisVault-v2.5.cdx.json`
- `SHA256SUMS`
